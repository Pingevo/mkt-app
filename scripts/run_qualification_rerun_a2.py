"""Evaluation-only: Rerun Agent 2 only after system prompt fix."""
...
QUAL_DIR = PROJECT_ROOT / "data" / "all_agents_beta_qualification" / f"beta_rerun_a2_{TIMESTAMP}"
...
def main():
    print("=" * 70)
    print("MKTApp Beta Qualification RERUN — Agent 2 only")
    print(f"Product fixture: {PRODUCT}")
    print(f"Fix: system prompt now allows web search URLs in default discovery mode")
    print(f"Qualification dir: {QUAL_DIR}")
    print("=" * 70)

    _init_session_baseline()

    print("\n--- A2: Default competitor discovery (rerun) ---")
    if budget_guard("A", 0.05, "A2_rerun"):
        ev = run_case(
            case_id="A2_default_discovery_rerun",
        ...
        print("A2 RERUN BLOCKED by budget")
